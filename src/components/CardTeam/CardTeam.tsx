...
export const CardTeam: FC<TCardTeamProps> = ({img, name, post, id, description}) => {
    return <div className="cardTeam" key={id}>
        <div className="cardTeam__textContent">
            <span className="cardTeam__teamName">{name}</span>
            <p className="cardTeam__teamPost">{post}</p>
            {description && <span className="cardTeam__education">{description}</span>}
        </div>
